...
export default function SignUp({ t: tProp, onSwitchMode }) {
  ...
  const STEPS = [
    t("signup.step_account"),
    t("signup.step_biz"),
    t("signup.step_security"),
    ...
    >
      ...
      {step === 0 && (
        <>
          ...
          >
            {t("signup.terms")}{" "}
            <BtnLink style={{ fontSize: 11 }} onClick={() => router.push("/terms")}>
              {t("signup.terms_link")}
            </BtnLink>{" "}
            {t("signup.and")}{" "}
            <BtnLink style={{ fontSize: 11 }} onClick={() => router.push("/privacy")}>
              {t("signup.privacy")}
            </BtnLink>
          </p>
        </>
  ...
  return (
    ...
    >
      ...
      <Field
        ...
        error={touched.company && errs.company}
      >
        <AuthInput
          type="text"
    ...
    >
      ...
      <Field label={t("signup.business_type")} error={touched.biz && err}>
        <AuthSelect
          ...
          onBlur={() => setTouched((p) => ({ ...p, biz: true }))}
          error={touched.biz && err}
          icon={<StoreIcon />}
        >
      ...
      >
        ...
        <p style={{ fontSize: 12.5, color: "var(--p-dark)", lineHeight: 1.65 }}>
          {t("signup.biz_hint")}
        </p>
      </div>
    ...
    >
      ...
      <div style={{ marginTop: 12 }}>
        <PasswordInput
          ...
          onBlur={() => setTouched((p) => ({ ...p, cpw: true }))}
          error={touched.cpw && cpwErr}
          icon={<LockIcon />}
        />
      </div>
...
function Step4({ email, onVerified, t }) {
  ...
  const verify = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/auth/verify-registration`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleResend = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/auth/resend-registration-otp`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    ...
    >
      ...
      >
        <span>
          {!canResend
            ? `${t("signup.otp_timer")} ${fmt(timer)}`
            : t("signup.otp_expired")}
        </span>

        <BtnLink
          style={{
            opacity: canResend && !resending ? 1 : 0.4,
