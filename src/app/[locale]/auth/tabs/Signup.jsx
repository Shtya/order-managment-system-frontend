	...
	return (
		<motion.div key="su1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: .25 }}>
			...
			<Field label={t('signup.email')} error={touched.email && errs.email}>
				<AuthInput type="text" placeholder={t('signup.email_placeholder')}
...
function Step4({ email, onVerified, t }) {
	...
	const verify = async () => {
		...
		try {
			// await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/auth/verify-otp`, {
			...
			toast.error(t('signup.otp_wrong'), { id: tid });
		...
		<motion.div key="su4" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: .25 }}>
			...
			<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '14px 0', fontSize: 13, color: 'var(--text-3)' }}>
				...
				<BtnLink style={{ opacity: canResend ? 1 : .4, pointerEvents: canResend ? 'auto' : 'none' }}
					onClick={() => { if (!canResend) return; setCanResend(false); setTimer(120); setOtp(''); setOtpErr(false); }}>
					{t('signup.otp_resend')}
				</BtnLink>
			</div>
	...
	return (
		<motion.div key="signup" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -14 }} transition={{ duration: .28 }}>
			...
			<AnimatePresence mode="wait">
				...
				{step === 2 && <Step3 key="s3" onNext={async d => { merge(d); await register(); }} onBack={() => setStep(1)} t={t} />}
				{step === 3 && <Step4 key="s4" email={formData.email} onVerified={() => { window.location.href = '/onboarding'; }} t={t} />}
			</AnimatePresence>
