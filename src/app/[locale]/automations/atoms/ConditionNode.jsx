...
export function ConditionNode({ id, data, selected }) {
    ...
    return (
        ...
        >
            <div className="text-[10px] text-slate-600 dark:text-slate-400 bg-purple-50/20 dark:bg-purple-500/5 p-2.5 rounded-xl border border-purple-100/30 dark:border-purple-500/10 min-h-[50px] flex flex-col justify-center">
                {loading ? (
                                    ...
                                    <>
                                        ...
                                        {data.config.checks.length > 2 && (
                                            <div className="text-[8px] text-slate-400 font-bold mt-0.5 text-center bg-slate-100 dark:bg-slate-800 rounded py-0.5">
                                                {t('nodes.otherConditions', { count: data.config.checks.length - 2 })}
                                            </div>
                                        )}
                                    </>
                                ) : (
                                    <div className="text-slate-400 italic text-center">{t('nodes.noConditions')}</div>
                                )}
                                <div className="flex items-center justify-center mt-0.5 opacity-50 font-bold text-[9px]">
                                    <span>{t('conditionTypes.order_check')}</span>
                                </div>
                            </div>
                        )}
                        {data.type === 'quick_order_status' && (
                            <div className="flex items-center justify-between">
                                <span className="opacity-50 font-bold">{t('config.status')}</span>
