...
import {
  ...
  RotateCcw, CreditCard, Info, Filter,
} from "lucide-react";
...
import {
  Card, ExportBtn, fmt, MiniTable, pct, PctBar,
  TableFilters, TrendChart, StatusDonut,
...
export default function DashboardPage() {
  ...
  const orderConfigs = [
    { key: "orders", label: t("chart.ordersCount"), color: SECONDARY,  fillOpacity: 0.1 },
    { key: "sales",  label: t("chart.sales"),       color: THIRD,      fillOpacity: 0.05, yAxisID: "y1" },
  ];

  const statsData = useMemo(() => KPI_CONFIG.map((card, i) => ({
    id: card.key,
    name: card.title,
    value: summary?.[card.key] == null
      ? "0"
      : card.pct ? pct(summary[card.key]) : fmt(summary[card.key]),
    icon: card.icon,
    color: card.color,
    sortOrder: i,
  })), [summary]);
