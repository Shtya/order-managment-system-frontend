...
const money = n => new Intl.NumberFormat("ar-EG",{style:"currency",currency:"EGP",maximumFractionDigits:0}).format(n??0);
...
function NumStat({ value, color, size=20, delay=0, plain=false }) {
  ...
  return (
    <span className="mono" style={{fontSize:size,fontWeight:500,color,lineHeight:1,letterSpacing:"-0.03em"}}>
      {plain?value:`${pre}${isNaN(n)?value:n.toLocaleString()}${suf}`}
    </span>
  );
}
...
export default function OrderConfirmationWorkPage() {
  ...
  const initOrder = data => {
    ...
    reset({ customerName:data.customerName||"",email:data.email||"",phoneNumber:data.phoneNumber||"",
  ...
  const handleSelectSku = useCallback(sku=>{
    ...
    setEditedOrder(prev=>recalc({...prev,items:[...prev.items,{isAdditional:true,variantId:sku.id,
  ...
  const cancelChanges = () => {
    ...
    reset({customerName:originalOrder.customerName||"",email:originalOrder.email||"",phoneNumber:originalOrder.phoneNumber||"",secondPhoneNumber:originalOrder.secondPhoneNumber||"",city:originalOrder.city||"",area:originalOrder.area||"",address:originalOrder.address||"",paymentMethod:originalOrder.paymentMethod||"",shippingCost:originalOrder.shippingCost??0,deposit:originalOrder.deposit??0,allowOpenPackage:originalOrder.allowOpenPackage??0});
  };
      ...
      <div style={{background:"var(--card)",borderBottom:"1px solid var(--border)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(148px,1fr))",gap:10,padding:"14px 24px",borderBottom:"1px solid var(--border)"}}>
          {[
            ...
            {icon:Banknote,   label:t("finalTotal"), value:money(order.finalTotal), color:HEX.orange, delay:.14},
            {icon:TrendingUp, label:t("profit"),     value:money(order.profit),     color:HEX.green,  delay:.18},
    ...
    <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:.3}}>
      <div className="bg-card !p-0 ">
        ...
        <ColHead open={open} onToggle={()=>setOpen(p=>!p)} icon={icon} color={color} eyebrow={eyebrow} title={title}
          right={
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span className="mono" style={{fontSize:11,color:"var(--muted-foreground)"}}>{totalQty} × {money(totalAmt)}</span>
                  ...
                  <table className=" ptable">
                        ...
                        return (
                          <motion.tr key={item.id||idx} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:idx*.035}}>
                            ...
                            <td style={{textAlign:"end"}}>
                              <span className="mono" style={{fontSize:12,color:"var(--muted-foreground)"}}>{money(item.unitPrice)}</span>
                            </td>

                            {/* Line total */}
                            <td style={{textAlign:"end",paddingInlineEnd:18}}>
                              <span className="mono" style={{fontSize:13,fontWeight:500,color:"var(--card-foreground)"}}>{money(item.unitPrice*item.quantity)}</span>
                            </td>
                    ...
                    <tfoot>
                      <tr>
                        ...
                        <td style={{textAlign:"end",paddingInlineEnd:18}}>
                          <span className="mono" style={{fontSize:14,fontWeight:500,color}}>{money(totalAmt)}</span>
                        </td>
                        <td />
                      </tr>
                    </tfoot>
        ...
        <div style={{padding:"16px 20px",display:"grid",gridTemplateColumns:"1fr 1fr",gap:13}}>
          ...
          <div className="field">
            ...
            <div style={{height:40,borderRadius:"var(--radius)",border:"1.5px dashed var(--border)",background:"var(--muted)",padding:"0 12px",display:"flex",alignItems:"center"}}>
              <span className="mono" style={{fontSize:14,fontWeight:500,color:remaining<0?HEX.red:HEX.green}}>{money(remaining)}</span>
            </div>
          </div>
