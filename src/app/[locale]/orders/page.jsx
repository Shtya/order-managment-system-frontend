		...
		<div className="flex items-center justify-between gap-4 flex-wrap">
			...
			<div className="flex items-center gap-2">
				<Button_
					...
					size="sm"
					label={t("toolbar.assignEmployee")}
					tone="purple"
					variant="solid"
				/>
					...
					<div className="space-y-4">
						<div>
							<h4 className="text-sm font-semibold text-gray-700 dark:text-slate-200 mb-3">
								{t("filters.customerInfo")}
							</h4>
							<div className="space-y-3">
								<div>
									<Label className="text-xs text-gray-500 dark:text-slate-400">
										{t("filters.customerName")}
									</Label>
									<Input
							...
							<div className="space-y-3">
								<div>
									...
									<Select
										...
										onValueChange={(v) => onChange({ ...value, orderType: v })}
									>
										<SelectTrigger className="w-full rounded-full !h-[40px] bg-[#fafafa] dark:bg-slate-800/50 mt-1">
											<SelectValue placeholder={t("filters.orderTypePlaceholder")} />
										</SelectTrigger>
										<SelectContent className="bg-card-select">
											<SelectItem value="all">{t("filters.orderTypeAll")}</SelectItem>
											<SelectItem value="distributed">{t("filters.orderTypeDistributed")}</SelectItem>
											<SelectItem value="normal">{t("filters.orderTypeNormal")}</SelectItem>
										</SelectContent>
									</Select>
								</div>
								<div>
...
export default function OrdersManagementPage() {
	...
	const getStatusBadgeStyle = (status) => {
		const styles = {
			"جديد": "bg-[#F3F6FF] text-[#6B7CFF] hover:bg-[#F3F6FF] dark:bg-blue-950/30 dark:text-blue-400",
			"ملغي": "bg-[#FEF2F2] text-[#EF4444] hover:bg-[#FEF2F2] dark:bg-red-950/30 dark:text-red-400",
			"قيد التنفيذ": "bg-[#FFF9F0] text-[#F59E0B] hover:bg-[#FFF9F0] dark:bg-orange-950/30 dark:text-orange-400",
			"مرتجع": "bg-[#FEF2F2] text-[#EF4444] hover:bg-[#FEF2F2] dark:bg-red-950/30 dark:text-red-400",
			"تم التسليم": "bg-[#F0FDF4] text-[#22C55E] hover:bg-[#F0FDF4] dark:bg-green-950/30 dark:text-green-400",
			"تم الارجاع": "bg-[#FEF2F2] text-[#EF4444] hover:bg-[#FEF2F2] dark:bg-red-950/30 dark:text-red-400",
			"خرج للشحن": "bg-[#F0F9FF] text-[#0EA5E9] hover:bg-[#F0F9FF] dark:bg-cyan-950/30 dark:text-cyan-400",
			"جاري الارجاع": "bg-[#FFF9F0] text-[#F59E0B] hover:bg-[#FFF9F0] dark:bg-orange-950/30 dark:text-orange-400",
			"تم الشحن": "bg-[#F0F9FF] text-[#0EA5E9] hover:bg-[#F0F9FF] dark:bg-cyan-950/30 dark:text-cyan-400",
		};
		return styles[status] || styles["جديد"];
	};
	...
	const [pager, setPager] = useState(() => ({
		...
		records: Array.from({ length: 13 }).map((_, i) => {
			const statuses = ["جديد", "ملغي", "قيد التنفيذ", "مرتجع", "تم التسليم", "تم الارجاع", "خرج للشحن", "جاري الارجاع", "تم الشحن"];
			return {
				id: i + 1,
				orderCode: "SRF56",
				customerName: "يسرا علام",
				phone: "201002766903",
				products: 5,
				orderStatus: statuses[i % statuses.length],
				paymentMethod: i % 3 === 0 ? "COD" : i % 3 === 1 ? "محفظة" : "انستا باي",
				assignedEmployee: "يسرا علام",
				distributor: "يسرا علام",
				trackingCode: "SRF56",
				city: "جدة",
				address: "جدة",
				shippingCompany: "جي تي",
				shippingCost: "500 د.أ",
				deposit: "300 د .أ",
				receivedAmount: "300 د .أ",
				date: "17-6-2025",
			};
		}),
	}));
				...
				cell: (row) => (
					<div className="flex items-center gap-2">
						<div className="flex -space-x-2">
							{[1, 2, 3].map((i) => (
								<div key={i} className="w-8 h-8 rounded-full bg-gray-200 dark:bg-slate-700 border-2 border-white dark:border-slate-800 flex items-center justify-center">
									<Package size={14} className="text-gray-500" />
								</div>
							))}
						</div>
						<span className="text-sm text-gray-600 dark:text-slate-300">+{row.products}</span>
					</div>
				),
			},
			{
				...
				cell: (row) => (
					<Badge className={cn("rounded-md", getStatusBadgeStyle(row.orderStatus))}>
						{row.orderStatus}
					</Badge>
				),
			},
				...
				cell: (row) => (
					<div className="flex items-center gap-2">
						<motion.button
							...
							whileTap={{ scale: 0.95 }}
							className="w-8 h-8 rounded-full bg-green-50 text-green-600 hover:bg-green-600 hover:text-white transition-colors flex items-center justify-center dark:bg-green-950/30 dark:hover:bg-green-600"
						>
							<Check size={16} />
						</motion.button>
						<motion.button
							whileHover={{ scale: 1.1 }}
							whileTap={{ scale: 0.95 }}
							className="w-8 h-8 rounded-full bg-red-50 text-red-600 hover:bg-red-600 hover:text-white transition-colors flex items-center justify-center dark:bg-red-950/30 dark:hover:bg-red-600"
						>
							<X size={16} />
						</motion.button>
					</div>
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<motion.button
										whileHover={{ scale: 1.1 }}
										whileTap={{ scale: 0.95 }}
										className="w-9 h-9 rounded-full border border-gray-200 bg-gray-50 text-gray-600 hover:bg-gray-600 hover:text-white transition-all flex items-center justify-center dark:bg-slate-800 dark:hover:bg-slate-700"
									>
										<X size={16} />
									</motion.button>
								</TooltipTrigger>
		...
		<div className="min-h-screen p-6">
			<div className="bg-card !pb-0 flex flex-col gap-2 mb-4">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2 text-lg font-semibold">
						...
						<span className="text-[rgb(var(--primary))]">{t("breadcrumb.orders")}</span>
			...
			<div className="bg-card rounded-sm">
				<OrdersTableToolbar
					...
					onAssignEmployee={() => console.log("assign employee")}
				/>
