              ...
              return (
                ...
                >
                  ...
                  <span
                    ...
                    onKeyDown={(e) =>
                      e.key === "Enter" && onRemove({ id: ri._originalItemId })
                    }
        ...
        {open && (
          ...
          >
            ...
            <div className="max-h-56 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800">
              {orderItems.map((item) => {
                const isSelected = replacementItems.some(
                  (ri) => ri._originalItemId === item.id,
                );
        ...
        <div className="space-y-2">
          ...
          {newSku ? (
            ...
            >
              ...
              <div className="min-w-0 flex-1 space-y-0.5">
                <p className="text-xs font-semibold text-gray-800 dark:text-gray-200 line-clamp-1">
                  {newSku.label || newSku.productName}
                </p>
                <p className="text-[10px] text-gray-400 dark:text-gray-500 font-mono">
                  {newSku.sku}
                </p>
...
function ReplacementItemsSection({
  ...
  const removeItem = (item) => {
    setReplacementItems((prev) =>
      prev.filter(
        (ri) => ri._originalItemId !== (item.id ?? item._originalItemId),
      ),
    );
  };
  ...
  return (
    <Section
      title={t("sections.replacementItems")}
      icon={ArrowLeftRight}
      delay={0.1}
    >
      {!selectedOrder ? (
                ...
                {replacementItems.length > 0 && (
                  <span
                    className="text-[10px] font-semibold px-2 py-0.5 rounded-full
										...
										border border-gray-200 dark:border-gray-700"
                  >
                    {replacementItems.length}/{orderItems.length}{" "}
                    {t("selected") || "selected"}
                  </span>
                )}
              </div>
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                  {t("selectFromDropdownAbove") ||
                    "Select items from the dropdown above to begin"}
                </p>
              </div>
            </motion.div>
          )}
...
function PriceSummaryCard({ replacementItems, form, selectedOrder, formatCurrency }) {
  ...
  const rows = [
    ...
    {
      label: t("summary.oldTotal"),
      value: formatCurrency(summary.oldTotal),
      color: "text-muted-foreground",
    },
    {
      label: t("summary.newTotal"),
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const data =
          existingData ??
          (await api.get(`/order-replacements/${replacementId}`)).data;
