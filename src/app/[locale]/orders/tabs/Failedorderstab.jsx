...
export function FailedOrdersTab() {
	...
	const handleRetry = useCallback(
		async (row) => {
			const isRetryable = row.status !== "success" && row.status !== "retrying";
			if (!isRetryable || retryingIds.includes(row.id)) return;

			setRetryingIds((prev) => [...prev, row.id]);
			...
			try {
				...
				setRetryingIds((prev) => prev.filter((id) => id !== row.id));
			}
		},
		[retryingIds, t, handleRefreshAll]
	);
    ...
    cell: (row) => {
        const isRetryable = row.status !== "success" && row.status !== "retrying";
        const isRetrying = retryingIds.includes(row.id);
        ...
        return (
            <ActionButtons
                ...
                actions={[
                    {
                        icon: isRetrying ? <Loader2 className="animate-spin" /> : <RotateCcw />,
                        tooltip: isRetrying
                            ? t("failedOrders.actions.retrying")
