			...
			<div className="bg-card !p-4   mt-4">
				<div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
					<div className="space-y-2">
						<Label>{t("filters.storageShelf")}</Label>
						<Input
							value={value.storage}
							onChange={(e) => onChange({ ...value, storage: e.target.value })}
							placeholder={t("filters.storagePlaceholder")}
					...
					<div className="space-y-2">
						...
						>
							...
							<SelectContent className={"bg-card-select"} >
								...
								<SelectItem value="home">{t("filters.categoryHome")}</SelectItem>
								<SelectItem value="medical">{t("filters.categoryMedical")}</SelectItem>
							</SelectContent>
						</Select>
					</div>
...
export default function ProductsPage() {
	...
	const [filters, setFilters] = useState({ storage: "", category: "all", sku: "" });

	/** ✅ Switcher */
	const items = useMemo(
		() => [
			{ id: "mine", label: t("tabs.mine"), icon: Box },
			{ id: "ship", label: t("tabs.ship"), icon: Box },
	...
	const stats = useMemo(
		() => [
			{
				title: t("stats.withShippingCompanies"),
				value: "76",
				icon: Package,
	...
	const [pager, setPager] = useState(() => ({
		...
		current_page: 1,
		per_page: 6,
		records: Array.from({ length: 6 }).map((_, i) => ({
			id: i + 1,
			storage: "B612",
			wholesalePrice: t("mock.wholesalePrice"),
			category: t("mock.category"),
			createdAt: "17-6-2025",
			name: t("mock.name"),
			sku: "SRF56",
		})),
		...
		return [
			{
				...
				header: t("table.sku"),
				cell: (row) => (
					<div className="inline-flex items-center gap-3 text-gray-500 dark:text-slate-300">
						<span className="text-gray-600 dark:text-slate-200">{row.sku}</span>
						<QrCode size={18} className="text-[rgb(var(--primary))]" />
					</div>
				),
			},
			{ key: "name", header: t("table.name"), className: "text-gray-700 dark:text-slate-200 font-semibold" },
			{ key: "wholesalePrice", header: t("table.wholesalePrice"), className: "text-gray-600 dark:text-slate-200" },
			{ key: "category", header: t("table.category"), className: "text-gray-500 dark:text-slate-300" },
			{
				key: "createdAt",
			...
			},
			{
				...
				cell: (row) => (
					<Badge className="rounded-md bg-[#EEF2FF] text-[rgb(var(--primary))] hover:bg-[#EEF2FF] dark:bg-slate-800 dark:text-[rgb(var(--primary))] dark:hover:bg-slate-800">
						{row.storage}
					</Badge>
				),
			},
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<motion.button
										...
										onClick={() => console.log("add", row.id)}
									>
										<Plus size={16} className="transition-transform group-hover:scale-125 group-hover:rotate-90" />
									</motion.button>
								</TooltipTrigger>
