				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
					...
					<div className="space-y-2">
						...
						<Select
							...
							onValueChange={(v) => onChange({ ...value, returnType: v })}
						>
							<SelectTrigger className=" w-full rounded-full !h-[45px] bg-[#fafafa]  dark:bg-slate-800/50">
							...
							<SelectContent className={"bg-card-select"}>
								...
								<SelectItem value="cash">{t("filters.returnTypeCash")}</SelectItem>
								<SelectItem value="bank">{t("filters.returnTypeBank")}</SelectItem>
								<SelectItem value="deduct">{t("filters.returnTypeDeduct")}</SelectItem>
							</SelectContent>
						</Select>
					</div>
...
export default function PurchaseReturnsPage() {
	...
	const [filters, setFilters] = useState({ status: "all", returnType: "all", supplier: "" });
	...
	const [pager, setPager] = useState(() => ({
		...
		records: Array.from({ length: 13 }).map((_, i) => ({
			...
			returnNumber: "SRF56",
			invoiceNumber: "SRF56",
			returnDate: "17-6-2025",
			status: i % 3 === 0 ? "مكتملة" : i % 3 === 1 ? "ملغاة" : "معلّقة",
			createdBy: "يسرا علام",
			supplierName: "نور صالح",
			returnType: i % 3 === 0 ? "كاش" : i % 3 === 1 ? "بنكي" : "خصم من حساب المورد",
			subtotal: "300 د.أ",
			tax: "300 د.أ",
			total: "300 د.أ",
		})),
	...
	const getStatusBadgeStyle = (status) => {
		const styles = {
			...
			"ملغاة": {
				className: "rounded-md bg-[#FEF2F2] text-[#EF4444] hover:bg-[#FEF2F2] dark:bg-red-950/30 dark:text-red-400 dark:hover:bg-red-950/30",
			},
			"معلّقة": {
		...
		return styles[status] || styles["معلّقة"];
	};
		...
		return [
			...
			},
			{
				...
				cell: (row) => {
					...
					return <Badge className={style.className}>{row.status}</Badge>;
				},
			},
			{
				key: "createdBy",
