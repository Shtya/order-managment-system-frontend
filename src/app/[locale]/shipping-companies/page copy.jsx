...
/** ✅ Toolbar (JSX) */
function ShippingTableToolbar({ t, searchValue, onSearchChange, onExport }) {
	return (
...
export default function ShippingCompaniesPage() {
	...
	const tabs = useMemo(() => [
		...
		{ id: "integrated", label: t("tabs.integrated") },
	], [t]);
	...
	const [pager, setPager] = useState(() => ({
		...
		records: Array.from({ length: 13 }).map((_, i) => ({
			id: i + 1,
			name: "جي تي",
			emiratesCount: i % 3 === 0 ? 5 : i % 3 === 1 ? 12 : 3,
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<motion.button
										...
										onClick={() => router.push(`/shipping-companies/details/${row.id}`)}
									>
			...
			<div className="bg-card !pb-0 flex flex-col gap-2 mb-4">
				...
				<div className="mt-6">
					...
					<SwitcherTabs
						items={tabs}
						activeId={activeTab}
						onChange={setActiveTab} className="w-full"
					/>
				</div>
			</div>
