...
import { TicketStatusBadge, TicketPriorityBadge } from "../atoms/TicketBadges";
import {
  useSupportTickets,
...
export default function TicketDetailPage() {
  ...
  const handleCancel = useCallback(
    async (reason) => {
      setMutating(true);
      try {
        const res = await cancelTicket(ticketId, reason);
        if (res) {
          await loadDetail();
          return true;
        }
    ...
    [cancelTicket, loadDetail, ticketId],
  );
  ...
  const breadcrumbs = useMemo(
    () => [
      { name: t("breadcrumb.home"), href: "/dashboard" },
      {
        ...
        onClick: () => router.push(`/${locale}/support-tickets`),
      },
  ...
  const isAdminOwn = false; // tenant view
            ...
            <div className="main-card rounded-2xl border border-border/50 p-4 space-y-3">
              ...
              <div className="space-y-2.5">
                  ...
                  value={
                    <div className="flex items-center gap-2 min-w-0">
                      <Avatar className="w-6 h-6 flex-shrink-0">
                        <AvatarImage src={avatarSrc(createdBy?.image)} />
                  ...
                  <SidebarRow
                    ...
                    value={
                      assigned ? (
                        <div className="flex items-center gap-2 min-w-0">
                          <Avatar className="w-6 h-6 flex-shrink-0">
                            <AvatarImage src={avatarSrc(assigned?.image)} />
                            <AvatarFallback className="text-[9px] font-bold">
                              {(assigned.name || "?").slice(0, 2).toUpperCase()}
                            </AvatarFallback>
                          </Avatar>
                          <span className="text-xs font-semibold text-foreground truncate">
                            {assigned.name}
                ...
                <SidebarRow
                  ...
                  textValue={format(ticket?.created_at)}
                />
                <SidebarRow
                  icon={<Clock className="w-4 h-4 text-muted-foreground" />}
                  label={t("filters.lastMessageAt")}
                  textValue={format(ticket?.lastMessageAt)}
                />
                <SidebarRow
                  icon={<Clock className="w-4 h-4 text-rose-500" />}
                  label={t("activity.last")}
                  textValue={format(ticket?.updated_at)}
                />
              </div>
            </div>
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <Avatar className="w-6 h-6">
                      <AvatarImage src={avatarSrc(lastActor.image)} />
                      <AvatarFallback className="text-[9px] font-bold">
                        {(lastActor.name || "AD").slice(0, 2).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                  </div>
                </div>
            ...
            <div className="main-card rounded-2xl border border-border/50 px-4 py-3 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className="min-w-0 flex-1 flex gap-1">
                  <h1 className="text-lg font-extrabold text-foreground leading-tight truncate max-w-[420px]">
                    {ticket?.title}
                  </h1>
                ...
                {TABS.map((tab) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "relative px-4 py-2 text-sm font-semibold transition-colors rounded-t-lg flex items-center gap-1.5",
                        active
                          ? "text-primary"
              ...
              <div className="flex-1 min-h-0 overflow-hidden">
                {activeTab === "messages" ? (
                  <TicketMessagesPanel
                    ...
                    onPreviewAttachment={(id, attachmentId) =>
                      openPreview(id, attachmentId)
                    }
                    onDownloadAttachment={(id, attachmentId, name) =>
                      downloadAttachment({
                        ticketId: id,
                        attachmentId,
                        originalName: name,
                    ...
                    /* Tight mode: we already render tabs + card outside the panel */
                    embedded
                  />
                ) : (
...
function SidebarRow({ icon, label, value, textValue, active }) {
  const t = null;
  return (
    <div
      className={cn(
        "flex items-center justify-between gap-3 py-1.5 border-b border-border/30 last:border-b-0",
      )}
    >
      ...
      <div className="min-w-0 text-start">
        {textValue ? (
          <span className="text-xs font-semibold text-foreground">{textValue}</span>
        ) : (
          value
        )}
      </div>
