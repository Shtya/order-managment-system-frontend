...
const styles = StyleSheet.create({
  ...
  brandIconBox: {
    width: 32,
    height: 32,
    backgroundColor: TEXT_DARK, // Match old charcoal feel for icon
    borderRadius: 8,
    display: "flex",
    alignItems: "center",
  ...
  colTrack: { flex: 2, fontFamily: DEFAULT_FONT_FAMILY, color: TEXT_MUTED },
  // Condition Footer
  conditionFooter: {
    padding: 10,
    backgroundColor: SKELETON_BG,
...
const ReturnPDF = ({ orders, carrier, employee, now, labels, locale }) => {
      ...
      <Page size="A4" style={[styles.page, { direction: isArabic ? "rtl" : "ltr", textAlign }]}>
        ...
        <View style={styles.headerBand}>
          <View style={[styles.headerTop, { flexDirection: flexDir }]}>
            <View style={[styles.headerBrand, { flexDirection: flexDir }]}>
              <View style={styles.brandIconBox}>
                <Svg viewBox="0 0 24 24" width={16} height={16} stroke={WHITE} fill="none" strokeWidth={1.8}>
              ...
              <View style={{ alignItems: isArabic ? "flex-end" : "flex-start" }}>
                ...
                <Text style={styles.docSubtitle}>RETURN RECEIPT · {carrier}</Text>
              </View>
            </View>
          ...
          <View style={[styles.metaStrip, { flexDirection: flexDir }]}>
            ...
            <View style={[styles.metaCell, { alignItems: isArabic ? "flex-end" : "flex-start" }]}>
              <Text style={styles.metaLabel}>{labels.returnDate}</Text>
        ...
        <View style={{ marginBottom: 20 }}>
          <Text style={[styles.sectionLabel, { textAlign }]}>{labels.ordersDetail}</Text>
          ...
          {orders.map((o, idx) => (
            <View key={o.code} style={styles.orderCard} wrap={false}>
              ...
              <View style={[styles.orderHead, { flexDirection: flexDir }]}>
                ...
                <Text style={styles.orderCity}>{o.city || "—"}</Text>
              </View>

              {/* Products Table */}
              <View>
                <View style={[styles.tableHeader, { flexDirection: flexDir }]}>
                  <Text style={[styles.th, styles.colSku, { textAlign }]}>{labels.sku}</Text>
                  <Text style={[styles.th, styles.colProduct, { textAlign }]}>{labels.product}</Text>
                  <Text style={[styles.th, styles.colQty]}>{labels.qty}</Text>
                  <Text style={[styles.th, styles.colTrack, { textAlign }]}>{labels.trackingCode}</Text>
                </View>
                ...
                {o.products.map((p, pIdx) => (
                  <View key={pIdx} style={[styles.tableRow, { flexDirection: flexDir }]}>
                    ...
                    <Text style={[styles.td, styles.colTrack, { textAlign }]}>{o.trackingNumber || "—"}</Text>
                  </View>
                ))}
              </View>

              {/* Condition Badge Block */}
              <View style={[styles.conditionFooter, { flexDirection: flexDir }]}>
                <Text style={styles.conditionText}>
                  {labels.condition}: {o.lastReturn?.reason || labels.conditions?.intact || "Intact"}
                </Text>
              </View>
