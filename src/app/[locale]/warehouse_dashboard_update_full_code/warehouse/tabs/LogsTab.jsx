...
function buildPreparedOrderBody(op, t, formatCurrency, currency) {
  ...
  return `
    ...
    <section class="surface">
      <div class="summary-strip">
        ...
        <div class="summary-box"><span>${t("pdf.orderPrice")}</span><b>${formatCurrency ? formatCurrency(order.total || getOrderValue(order)) : `${order.total || getOrderValue(order)} ${currency}`}</b></div>
      </div>
    </section>
    ...
    ${errorsSection}
    ${buildOrderSummarySection([order], { title: t("pdf.finalSummary"), totalOrders: t("pdf.totalOrders"), totalSkus: t("pdf.totalSkus"), totalItems: t("pdf.totalItems"), totalValue: t("pdf.totalValue") }, formatCurrency, currency)}
  `;
}

function buildPreparedSessionBody(sessionOps, t, formatCurrency, currency) {
  ...
  const orderBlocks = sessionOps.map((operation) => buildPreparedOrderBody(operation, t, formatCurrency, currency)).join("");
  ...
  return `
    ...
    ${orderBlocks}
    ${buildOrderSummarySection(orders, { title: t("pdf.finalSummary"), totalOrders: t("pdf.totalOrders"), totalSkus: t("pdf.totalSkus"), totalItems: t("pdf.totalItems"), totalValue: t("pdf.totalValue") }, formatCurrency, currency)}
  `;
}

function buildGenericBody(op, order, t, formatCurrency, currency) {
  return `
    <section class="hero hero-info">
    ...
    ${order ? `
      <section class="surface">
        ${buildOrderOverviewCards(order, { customer: t("pdf.customer"), city: t("pdf.city"), orderValue: t("pdf.orderValue"), orderSummary: t("pdf.orderSummary"), itemsWord: t("common.itemsWord") }, formatCurrency, currency)}
      </section>
      ${buildOrderSummarySection([order], { title: t("pdf.finalSummary"), totalOrders: t("pdf.totalOrders"), totalSkus: t("pdf.totalSkus"), totalItems: t("pdf.totalItems"), totalValue: t("pdf.totalValue") }, formatCurrency, currency)}
    ` : ""}
  `;
      ...
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto rounded-3xl  p-0" dir="rtl">
        ...
        <div className="space-y-5 px-6 py-6">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            {isPreparedLog ? (
              <>
                <Button onClick={() => openPdfDocument({ title: t("pdf.preparedTitle"), filename: `${log.orderCode}_prepared.pdf`, body: buildPreparedOrderBody(log, t, formatCurrency, currency) })}>{t("actions.openOrderPdf")}</Button>
                {sessionLogs.length > 1 ? <Button variant="outline" onClick={() => openPdfDocument({ title: t("pdf.sessionTitle"), filename: `prepared_session_${log.createdAt?.slice(0, 10) || "session"}.pdf`, body: buildPreparedSessionBody(sessionLogs, t, formatCurrency, currency) })}>{t("actions.openSessionPdf")}</Button> : null}
              </>
            ) : (
              <Button onClick={() => openPdfDocument({ title: t("pdf.operationTitle"), filename: `${log.id}.pdf`, body: buildGenericBody(log, relatedOrder, t, formatCurrency, currency) })}>{t("actions.openPdf")}</Button>
            )}
