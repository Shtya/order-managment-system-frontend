...
function InfoCard({
	...
	// Per-stat accent colour (hex). Falls back to CSS --primary token.
	const accentHex = customStyles?.iconColor || null;
	const accent = accentHex || "var(--primary)";

	// All tints are derived from the accent so they auto-adapt in dark mode
	// when accentHex is absent (falls back to CSS var which already switches).
	...
	const topBarEnd = accentHex ? `${accentHex}55` : "color-mix(in oklab, var(--primary) 38%, transparent)";
	...
	return (
		...
		>
			{/* ── Left side accent tab (key first-design signature) ── */}
			<div style={{
			...
			<div style={{
				...
				opacity: hov ? 1 : 0,
				background: `radial-gradient(ellipse at 15% 15%, ${accentHex ? accentHex + "0d" : "color-mix(in oklab, var(--primary) 6%, transparent)"
					}, transparent 65%)`,
			...
			<div style={{
				...
				<div style={{ flex: 1, minWidth: 0 }}>
					{/* Instrument Serif display number — the editorial signature */}
					<div style={{
					...
					<>
						...
						<AnimatePresence>
							{hov && (
								...
								>
									{/* Edit btn — uses --primary tint */}
									<button
										onClick={(e) => { e.stopPropagation(); onEdit?.(); }}
										style={{
											width: 28, height: 28, borderRadius: 8, outline: "none",
										...
										onMouseLeave={e => {
											e.currentTarget.style.background = "color-mix(in oklab, var(--primary) 10%, var(--card))";
									...
									<button
										...
										onMouseLeave={e => {
											e.currentTarget.style.background = "color-mix(in oklab, var(--destructive) 10%, var(--card))";
...
function SwitcherTabs({
	...
	return (
		<div
			...
			className={["flex items-stretch w-full", className].join(" ")}
		>
			{items.map((item) => {
				...
				return (
					...
					>
						...
						{/* Active underline — primary → secondary brand gradient */}
						{isActive && (
							<motion.span
		...
		<div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))" }}>
			{Array.from({ length: count }).map((_, i) => (
				<motion.div
					...
					/* uses the project's .skeleton class from globals.css
						 which already handles light/dark shimmer */
								...
								return (
									<li key={i} className="flex items-center gap-2">
										...
										{isLast ? (
											...
											>
												{/* Instrument Serif — editorial display title */}
												<span
												...
												{/* Live dot — uses --primary, auto-switches in dark mode */}
