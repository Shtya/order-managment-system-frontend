  ...
  const MenuItem = ({ item, index }) => {
    ...
    const menuItemContent = (
      <motion.div
        ...
        transition={
          didMount ? { duration: 0 } : { delay: index * 0.05, duration: 0.3 }
        }
        whileTap={{ scale: 0.97 }}
      ...
      >
        <div className="w-full">
          ...
          >
            ...
            <AnimatePresence>
              {isOpen && (
                <motion.div className="flex-1 flex items-center justify-between relative z-10">
                  <span
                    className={`font-medium text-sm ${active ? "font-bold" : ""} transition-all duration-200`}
                  >
                    {t(item.labelKey)}
                  </span>
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    {hasChildren && (
                      ...
                      >
                        <FaChevronDown
                          className={` ${isOpenGroup ? "rotate-180" : ""} rotate-0 duration-300`}
                          size={14}
                        />
                      </div>
                    )}
          ...
          {isOpen && (
            <div
              style={{ transition: ".5s" }}
              className={`  overflow-hidden   max-h-0 ${isOpenGroup ? "max-h-[500px] " : " "}  `}
            >
                  ...
                  <AnimatePresence>
                    {isOpen && (
                      ...
                      >
                        ...
                        <FaChevronRight
                          size={12}
                          className={`${isRTL ? "rotate-180" : ""} opacity-70`}
                        />
                      </motion.div>
                    )}
                  </AnimatePresence>
