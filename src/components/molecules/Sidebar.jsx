...
import {
	...
	Activity, Printer, CheckCircle2, RefreshCw, ClipboardList,
	Banknote, AlertCircle, PieChart, Package2,
	Menu,
	X,
} from 'lucide-react';
...
const menuItems = [
...

	{
		...
		children: [
			{ icon: PieChart, labelKey: 'order-analysis', href: '/reports/order-analysis' },
  ...
  `;
						...
						<div className="flex items-center gap-1 shrink-0 ml-1">
							{item.badge && (
								<span
									...
									style={{ background: "linear-gradient(135deg, var(--third), var(--primary))" }}
								>
									{item.badge}
								</span>
							)}
	...
	if (hasChildren) {
		return wrappedInTooltip(
			<button
				ref={ref}
				onClick={(e) => { addRipple(e, ref); if (!isOpen) onOpenSidebar?.(); else onToggle(item.href); }}
				className={sharedClass}
				style={active ? activeStyle : inactiveStyle}
				onMouseEnter={e => { if (!active) e.currentTarget.style.background = "color-mix(in oklab, var(--primary) 4%, var(--card))"; }}
	...
	return wrappedInTooltip(
		<Link
			ref={ref}
			href={item.href}
			onClick={(e) => { addRipple(e, ref); if (!isOpen) { e.preventDefault(); onOpenSidebar?.(); } }}
			className={sharedClass}
			style={active ? activeStyle : inactiveStyle}
			onMouseEnter={e => { if (!active) e.currentTarget.style.background = "color-mix(in oklab, var(--primary) 4%, var(--card))"; }}
			onMouseLeave={e => { if (!active) e.currentTarget.style.background = "transparent"; }}
...
function SubItem({ child, isActive, isRTL, index }) {
	...
	return (
        ...
        `}
				...
				onMouseEnter={e => { if (!active) e.currentTarget.style.background = "color-mix(in oklab, var(--primary) 3.5%, var(--card))"; }}
				onMouseLeave={e => { if (!active) e.currentTarget.style.background = "transparent"; }}
			>
				<span
					className={`absolute ${isRTL ? 'right-0' : 'left-0'} top-[20%] h-[60%] w-[2px] rounded-full transition-all duration-200`}
					style={{ background: active ? "linear-gradient(180deg, var(--primary), var(--third))" : "transparent" }}
				/>
...
const Sidebar = ({ isOpen, isRTL, onOpenSidebar }) => {
	...
	useEffect(() => {
		const active = menuItems.find(item => item.children?.some(c => isActive(c.href)));
	...
	return (
        ...
        `}
			...
			>
				...
				<motion.div className={`${!isOpen ? "mx-auto pe-[7px] " : "ps-4 flex items-center gap-3 "} my-[11.6px] `} whileTap={{ scale: 0.92 }}>
					<Button
					...
					{isOpen && <div className={`flex items-center gap-2.5 `}>
