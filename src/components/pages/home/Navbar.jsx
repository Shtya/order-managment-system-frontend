...
import { Globe, LayoutDashboard, LogOut, Crown, BadgeCheck, ChevronDown, Zap } from "lucide-react";
...
function getRoleBadgeStyle(role) {
	if (role === "super_admin") return { background: "#fef3c7", color: "#d97706", border: "1px solid #fde68a" };
	if (role === "admin") return { background: `${BRAND}14`, color: BRAND, border: `1px solid ${BRAND}30` };
	return { background: "#f0fdf4", color: "#16a34a", border: "1px solid #bbf7d0" };
}

function getPlanStatusStyle(status) {
	if (status === "active") return { background: "#f0fdf4", color: "#16a34a", dot: "#22c55e" };
	if (status === "cancelled") return { background: "#fef2f2", color: "#dc2626", dot: "#ef4444" };
...
function LanguageToggle({ currentLang, onToggle, languages = { ar: "AR", en: "EN" } }) {
	...
	return (
		<motion.button
			type="button" onClick={onToggle} whileTap={{ scale: 0.93 }}
		...
		>
			<AnimatePresence>
				{hovered && (
					<motion.span initial={{ x: "-100%" }} animate={{ x: "250%" }} exit={{ opacity: 0 }}
						transition={{ duration: 0.55 }} className="absolute inset-0 pointer-events-none skew-x-12"
						style={{ background: `linear-gradient(90deg,transparent,${BRAND}22,transparent)` }} />
				)}
			</AnimatePresence>
			<motion.div animate={{ rotate: hovered ? 180 : 0 }} transition={{ duration: 0.4 }}>
				<Globe size={13} style={{ color: hovered ? BRAND : "#9ca3af" }} />
			</motion.div>
			<span className="w-px h-3.5 bg-gray-200" />
			<AnimatePresence mode="wait">
				<motion.span key={otherLang} initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -6, opacity: 0 }}
					transition={{ duration: 0.14 }} className="text-[11px] font-bold uppercase tracking-widest"
...
function AvatarPopover({ user, t }) {
	...
	const planStyle = user.plan ? getPlanStatusStyle(user.plan.status) : null;
	...
	return (
		<div ref={ref} className="relative">
			...
			>
				{/* Avatar */}
				<div className="w-6 h-6 rounded-xl flex items-center justify-center text-[10px] font-black text-white flex-shrink-0 relative overflow-hidden"
					style={{ background: `linear-gradient(135deg,${BRAND},${BRAND_L})` }}>
					<motion.div animate={{ x: ["-120%", "220%"] }} transition={{ duration: 3, repeat: Infinity, repeatDelay: 4 }}
						className="absolute inset-0 skew-x-12" style={{ background: "linear-gradient(90deg,transparent,rgba(255,255,255,0.4),transparent)" }} />
					<span className="relative z-10">{initials}</span>
				</div>
				<div className="flex flex-col items-start leading-none gap-0.5">
					<span className="text-[12px] font-bold text-gray-800 max-w-[72px] truncate">{user.name}</span>
					<span className="text-[9.5px] font-semibold uppercase tracking-wide" style={{ color: BRAND }}>
				...
				{open && (
					<motion.div
						...
						style={{
							...
							boxShadow: "0 24px 64px rgba(103,99,175,0.2), 0 4px 16px rgba(0,0,0,0.06)",
							backdropFilter: "blur(28px)",
							overflow: "hidden",
						}}
					>
						{/* gradient top bar */}
						<div style={{ height: 3, background: `linear-gradient(90deg,${BRAND},${BRAND_L},#a78bfa,${BRAND})`, backgroundSize: "200% 100%" }} />

						{/* User header */}
						<div className="px-4 pt-4 pb-3">
							<div className="flex items-center gap-3">
								<div className="w-10 h-10 rounded-xl flex items-center justify-center text-base font-black text-white flex-shrink-0 relative overflow-hidden"
									style={{ background: `linear-gradient(135deg,${BRAND},${BRAND_L})`, boxShadow: `0 6px 18px ${BRAND}40` }}>
									<motion.div animate={{ x: ["-120%", "220%"] }} transition={{ duration: 3.5, repeat: Infinity, repeatDelay: 3 }}
										className="absolute inset-0 skew-x-12" style={{ background: "linear-gradient(90deg,transparent,rgba(255,255,255,0.35),transparent)" }} />
									<span className="relative z-10">{initials}</span>
								</div>
								<div className="flex-1 flex items-center gap-2 justify-between min-w-0 pt-0.5">
									<div>
										<div className="flex items-center gap-1.5">
											<span className="text-[14px] font-extrabold text-gray-900 truncate leading-tight">{user.name}</span>
											<BadgeCheck size={13} style={{ color: BRAND, flexShrink: 0 }} />
										</div>
										<p className="text-[11px] text-gray-400 truncate mt-0.5">{user.email}</p>
									</div>
									{/* Role */}
									<span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-xl" style={roleStyle}>
										<Crown size={8} />
										{getRoleLabel(user.role)}
									</span>
								</div>
						...
						<div className="mx-3 h-px" style={{ background: "linear-gradient(90deg,transparent,#ede9fe,transparent)" }} />
						...
						<div className="p-2">
							{[
								...
								const sharedClass = "flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl transition-all duration-150 text-gray-700";
								return item.isLink ? (
									<Link key={idx} href={item.href} onClick={() => setOpen(false)}
										className={sharedClass}
										onMouseEnter={(e) => { e.currentTarget.style.background = item.hoverBg; e.currentTarget.style.color = item.hoverColor; }}
										onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#374151"; }}>
										{inner}
									</Link>
								) : (
									<button key={idx} onClick={item.onClick} className={sharedClass}
										onMouseEnter={(e) => { e.currentTarget.style.background = item.hoverBg; e.currentTarget.style.color = item.hoverColor; }}
										onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#374151"; }}>
										{inner}
									</button>
...
export default function Navbar({ t, locale, switchLocale, user }) {
	...
	const navLinks = [t("nav.home"), t("nav.services"), t("nav.about"), t("nav.pricing"), t("nav.contact")];
		...
		<>
			...
			>
				...
				>
						...
						<div className="flex items-center justify-between h-16">
							...
							<motion.div whileHover={{ scale: 1.02 }} className="flex items-center gap-2.5 cursor-pointer select-none">
								<div className="w-9 h-9 rounded-xl flex items-center justify-center overflow-hidden relative"
									style={{ background: `linear-gradient(135deg,${BRAND},#8b88c9)`, boxShadow: `0 4px 14px ${BRAND}44` }}>
									<motion.div className="absolute inset-0 -skew-x-12"
										animate={{ x: ["-150%", "250%"] }} transition={{ duration: 3.5, repeat: Infinity, repeatDelay: 2 }}
							...
							<div className="hidden md:flex items-center gap-0.5">
								{navLinks.map((link, i) => {
									...
									return (
										<motion.button key={link} onClick={() => setActiveLink(link)} {...stagger(i)}
											whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
											className="relative px-4 py-2 text-[13px] font-medium rounded-xl transition-colors"
											style={{ color: isActive ? BRAND : "#6b7280" }}>
											{link}
											{isActive && (
												<motion.div layoutId="activeLink" className="absolute inset-0 rounded-xl -z-10"
							...
							<div className="hidden md:flex items-center gap-2">
								<LanguageToggle currentLang={locale} languages={{ ar: "AR", en: "EN" }}
								...
								{user ? (
									...
									<>
										<Link href="/auth?mode=signup"
											...
											style={{ color: BRAND, borderColor: `${BRAND}35`, background: `${BRAND}06` }}>
											{t("auth.signup")}
										</Link>
										<Link href="/auth?mode=signin"
											className="relative text-[12.5px] font-bold px-4 py-2 rounded-xl text-white overflow-hidden"
											style={{ background: `linear-gradient(135deg,${BRAND} 0%,#8b88c9 100%)`, boxShadow: `0 4px 16px ${BRAND}35` }}>
							...
							<div className="md:hidden flex items-center gap-2">
								...
								<motion.button whileTap={{ scale: 0.92 }} onClick={() => setMobileOpen((v) => !v)}
									aria-label="Toggle menu" className="w-9 h-9 flex flex-col items-center justify-center gap-[5px] rounded-xl">
									<motion.span animate={mobileOpen ? { rotate: 45, y: 7 } : { rotate: 0, y: 0 }} transition={{ duration: 0.25 }}
										className="block w-5 h-0.5 rounded-full origin-center" style={{ background: BRAND }} />
									<motion.span animate={mobileOpen ? { opacity: 0, scaleX: 0 } : { opacity: 1, scaleX: 1 }} transition={{ duration: 0.2 }}
										className="block w-5 h-0.5 rounded-full" style={{ background: BRAND }} />
									<motion.span animate={mobileOpen ? { rotate: -45, y: -7 } : { rotate: 0, y: 0 }} transition={{ duration: 0.25 }}
										className="block w-5 h-0.5 rounded-full origin-center" style={{ background: BRAND }} />
								</motion.button>
							</div>
					...
					>
						<div className="container mx-auto px-6 py-4 flex flex-col gap-1" dir={locale === "ar" ? "rtl" : "ltr"}>
							...
							{user && (
								<motion.div
									...
									style={{ background: `${BRAND}08`, border: `1px solid ${BRAND}18` }}
								>
							...
							{navLinks.map((link, i) => {
								const isActive = activeLink === link;
								return (
									<motion.button key={link} initial={false}
										animate={mobileOpen ? { opacity: 1, x: 0 } : { opacity: 0, x: locale === "ar" ? 16 : -16 }}
										...
										style={{ textAlign: locale === "ar" ? "right" : "left", color: isActive ? BRAND : "#6b7280", background: isActive ? `${BRAND}10` : "transparent" }}>
										{link}
									</motion.button>
								);
							})}

							<div className="flex gap-2 mt-2 pt-3" style={{ borderTop: `1px solid ${BRAND}12` }}>
								{user ? (
									<>
										<Link href={getDashboardRoute(user.role)} onClick={() => setMobileOpen(false)}
