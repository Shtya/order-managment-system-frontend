...
/* ─── Scrolling Column ─── */
function ScrollColumn({ reviews, duration, className = "", reverse = false }) {
	// Duplicate for infinite loop
	const doubled = [...reviews, ...reviews];

	return (
		<div style={{ flex: 1, position: 'relative', minWidth: 0 }} className={` lg:overflow-hidden ${className}`}>
			{/* Top fade */}
	...
	return (
      ...
      `}</style>
			...
			>
				...
				{/* 3 columns */}
				<div className='flex flex-col lg:flex-row lg:h-[560px]' style={{
					...
					<ScrollColumn reviews={col2} duration={28} reverse={true} className="max-lg:hidden" />
					<ScrollColumn reviews={col3} duration={20} reverse={false} className="max-lg:hidden" />
				</div>
			</section>
		</>
	);
